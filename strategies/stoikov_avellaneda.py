...
from simulator.utils import MdUpdate, Order, OwnTrade, update_best_positions
...
class StoikovStrategy(BaseStrategy):
    ...
    def _calc_theta(self):
        if self._theta_policy == 'std':
            ...
            theta = np.mean(np.asarray(
                self.queues['best_ask']) - np.asarray(self.queues['best_bid']))
        else:
...
class FutureStrategy(BaseStrategy):
    ...
    def __init__(self, delay: float,
        ...
        self.future_mid_price = 0.5 * \
            (self.future_bid_price + self.future_ask_price)
        self.future_mid_price = get_mid_price(
            self.future_mid_price, self.future_md)
