...
class Sim:
    ...
    def update_md(self, md: MdUpdate) -> None:
        ...
        self.best_bid, self.best_ask = update_best_positions(
            self.best_bid, self.best_ask, md)
    ...
    def execute_orders(self) -> None:
        ...
        executed_orders_id += (
            self.bid_orders.greater_or_eq(self.trade_price['ASK']))
        executed_orders_id += (
            self.ask_orders.less_or_eq(self.trade_price['BID']))
    ...
    def place_order(self, ts: float, size: float, side: str, price: float) -> Order:
        ...
        order = Order(ts, ts + self.latency,
                      self.get_order_id(), side, size, price)
